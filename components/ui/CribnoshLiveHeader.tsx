...
export function CribnoshLiveHeader({
  ...
  return (
    <View className="flex-row items-start justify-between rounded-xl overflow-hidden px-2 py-1.5 w-full bg-transparent">
      {/* Left side: Text column first, then avatar */}
      <View className="flex-row items-start flex-1 min-w-0">
        <View className="flex-1 flex-col items-start justify-center min-w-0 gap-y-0.5">
        ...
        <Avatar
          ...
          size="md"
          className="w-[64px] h-[64px] rounded-full bg-[#CEC9FF] mt-4 ml-3"
        />
      </View>
    </View>
  );
