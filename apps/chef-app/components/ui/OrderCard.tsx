...
export const OrderCard: React.FC<OrderCardProps> = ({
  ...
  const calculateChefEarnings = (total: number): number => {
    const platformFee = Math.round(total * 0.15);
    return total - platformFee;
  };

  const chefEarnings = totalAmount ? calculateChefEarnings(totalAmount) : null;
        ...
        <View style={styles.orderFooter}>
          <View style={styles.priceContainer}>
            <View>
              ...
              {showChefEarnings && chefEarningsDisplay && (
                <Text style={styles.chefEarningsText}>
                  You earn: {chefEarningsDisplay}
                </Text>
              )}
            </View>
          </View>
