...
async function handlePOST(request: NextRequest) {
  try {
    ...
    console.log(`Order ${orderId} marked as delivered by ${payload.user_id} (${payload.role})`);

    return ResponseFactory.success({});
  } catch (error: any) {
    console.error('Error delivering order:', error);
    return ResponseFactory.internalError('Failed to deliver order');
  }
}
