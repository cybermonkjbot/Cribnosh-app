...
async function handleGET(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    return ResponseFactory.internalError(error.message || 'Failed to fetch connections.');
  }
}
...
async function handlePOST(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    return ResponseFactory.internalError(error.message || 'Failed to create connection.');
  }
}
