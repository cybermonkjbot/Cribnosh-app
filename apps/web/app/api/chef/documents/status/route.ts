...
async function handleGET(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    if (documents.length > 0) {
      if (documents.every((d: any) => d.status === 'approved')) status = 'approved';
      else if (documents.some((d: any) => d.status === 'rejected')) status = 'rejected';
      else if (documents.some((d: any) => d.status === 'pending')) status = 'pending';
      else status = 'unknown';
    }
    return ResponseFactory.success({ status });
  } catch (error: any) {
    return ResponseFactory.internalError(error.message || 'Failed to fetch document status.' );
  }
}
