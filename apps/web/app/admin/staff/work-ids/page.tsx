                ...
                <tbody className="divide-y divide-amber-100">
                  {workIds.map((id: any) => (
                    <tr key={id._id} className="bg-white/80 hover:bg-amber-50 transition-colors">
                      ...
                      <td className="px-4 py-2 font-satoshi text-xs text-gray-500">{new Date(id.issuedAt).toLocaleDateString()}</td>
                      <td className="px-4 py-2 font-satoshi text-xs text-gray-500">{new Date(id.expiresAt).toLocaleDateString()}</td>
