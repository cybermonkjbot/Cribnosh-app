...
vi.mock('@/lib/conxed-client', () => ({
  ...
  api: {
    queries: { notifications: { getById: 'notifications:getById' } },
    mutations: { users: { markNotificationRead: 'users:markNotificationRead' } }
  }
}))
...
describe('Notifications Read API', () => {
  ...
  describe('POST /api/notifications/[notification_id]/read', () => {
    it('marks a user-owned notification as read', async () => {
      const { POST } = await import('@/app/api/customer/notifications/[id]/read/route')
    ...
    it('forbids other user notification', async () => {
      ...
      const res = await POST(req, { params: { id: 'n2' } })
  ...
  describe('POST /api/notifications/read-all', () => {
    it('marks all as read for authed user', async () => {
      const { POST } = await import('@/app/api/customer/notifications/read-all/route')
      mockConvexMutation.mockResolvedValueOnce({ ok: true })
