...
export default function SelectMealScreen() {
  ...
  useEffect(() => {
    if (currentSelections?.order_items) {
      setSelectedItems(currentSelections.order_items.map((item: any) => ({
  ...
  const handleSaveSelections = async () => {
    ...
    try {
      ...
      Alert.alert('Error', error?.data?.message || 'Failed to update selections');
    } finally {
      setIsSubmitting(false);
    }
  };
  
  if (isLoadingOrder || isLoadingSelections) {
    return (
  ...
  // TODO: Fetch actual menu items from chef_id
  // For now, showing a placeholder message
  const menuItems: any[] = []; // This should be fetched from the chef's menu
      ...
      <ScrollView style={styles.content} contentContainerStyle={styles.contentContainer}>
        ...
        <View style={styles.infoCard}>
          ...
          <Text style={styles.infoText}>
            Select items from the menu. Your selections will be shared with the group.
          </Text>
        </View>
        ...
        {selectedItems.length > 0 ? (
          ...
          <View style={styles.emptyContainer}>
            ...
            <Text style={styles.emptySubtext}>
              TODO: Add menu items from {groupOrder.restaurant_name}
            </Text>
          </View>
        )}
      </ScrollView>
