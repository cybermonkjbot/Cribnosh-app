...
export function AddressSelectionModal({
  ...
  // Refetch profile when modal opens to get latest addresses
  useEffect(() => {
    if (isAuthenticated) {
      fetchProfile();
    }
  ...
  const savedAddresses = useMemo<SavedAddress[]>(() => {
    ...
    // Handle both data structures: profileData?.data?.user or profileData?.data
    const profile = profileData?.data?.user || profileData?.data;

    if (profile?.address) {
      // Normalize address - handle both zipCode and postal_code formats
      const address = profile.address;
      const normalizedAddress: CustomerAddress = {
        street: address.street || '',
        city: address.city || '',
        state: address.state || '',
        postal_code: address.postal_code || (address as any).zipCode || '',
        country: address.country || '',
        coordinates: address.coordinates,
      };

      // Only add if address has required fields
