...
interface CaloriesNoshPointsCardsProps {
  ...
  availableCoins?: number; // Available Nosh Points/coins
}
...
const MemoizedCaloriesNoshPointsCards: React.FC<CaloriesNoshPointsCardsProps> = React.memo(({
  ...
  const triggerSwipeAnimation = () => {
    // Fade in the cue
    cueOpacity.value = withTiming(1, { duration: 300 }, (finished) => {
      'worklet';
      if (finished) {
        // Swipe right animation
        swipeAnimation.value = withTiming(1, { duration: 600 }, (finished2) => {
          'worklet';
          if (finished2) {
            // Pull back animation
            swipeAnimation.value = withTiming(0, { duration: 400 }, (finished3) => {
              'worklet';
              if (finished3) {
                // Fade out the cue
                cueOpacity.value = withTiming(0, { duration: 300 }, (finished4) => {
                  'worklet';
                  if (finished4) {
                    runOnJS(setShowSwipeCue)(false);
                    runOnJS(setArrowTapCount)(0);
                  }
                });
              }
            });
  ...
  const activeCardAnimatedStyle = useAnimatedStyle(() => {
    const translateX = interpolate(swipeAnimation.value, [0, 1], [0, 30]);
    return {
      transform: [
  ...
  return (
    <View style={styles.container}>
      ...
      <Animated.View style={[
        ...
        isCaloriesOnTop ? activeCardAnimatedStyle : {}
      ]}>
        <Pressable
        ...
        >
          <BlurView intensity={27.5} style={styles.blurOverlay}>
            <View style={styles.noshPointsContent}>
              ...
              {/* Nosh Points Amount */}
              <Text style={styles.noshPointsAmount}>
                {availableCoins !== undefined ? availableCoins.toLocaleString() : '1,240'}
              </Text>
