...
export function AddCardSheet({
  ...
  const { addPaymentMethod, getPaymentMethods, createSetupIntent } =
    usePayments();
  ...
  if (__DEV__) {
    console.log("AddCardSheet - Stripe Status:", {
      ...
      publishableKeyPrefix:
        STRIPE_CONFIG.publishableKey?.substring(0, 20) || "MISSING",
  ...
  const handleAddCard = async () => {
    ...
    if (!stripe || !confirmSetupIntent) {
      showToast({
        ...
        message:
          "Stripe is not properly configured. Please check your environment variables and restart the app.",
        duration: 5000,
      });
    ...
    if (
      ...
      STRIPE_CONFIG.publishableKey.length === 0
    ) {
      showToast({
        type: "error",
    ...
    try {
      ...
      if (!setupIntentResult || !setupIntentResult.data?.clientSecret) {
      ...
      if (__DEV__) {
        ...
        const publishableKeyAccountId =
          STRIPE_CONFIG.publishableKey?.substring(8, 24) || "N/A";
        ...
        console.log("Setup Intent Details:", {
          ...
          publishableKeyPrefix:
            STRIPE_CONFIG.publishableKey?.substring(0, 20) || "N/A",
          publishableKeyLength: STRIPE_CONFIG.publishableKey?.length || 0,
        ...
        if (!accountsMatch) {
          console.error(
            "⚠️ ACCOUNT MISMATCH: Publishable key and setup intent are from different Stripe accounts!",
          );
          console.error(
            '   This will cause "API key" errors. Ensure both keys are from the same account.',
          );
        }
      }
      ...
      if (!clientSecret || !clientSecret.startsWith("seti_")) {
        ...
        throw new Error(
          "Invalid setup intent client secret received from server",
        );
      }
      ...
      if (!confirmSetupIntent) {
        throw new Error(
          "Stripe confirmSetupIntent is not available. Please ensure StripeProvider is properly configured with a valid publishable key.",
        );
      }
        ...
        <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
          ...
          <View style={styles.section}>
            ...
            <Text style={styles.hintText}>
              Enter your card details securely. We use Stripe to process your
              payment information.
            </Text>
          </View>
          ...
          <View style={styles.section}>
            <View style={styles.setDefaultContainer}>
              <View style={styles.setDefaultLeft}>
                <Text style={styles.setDefaultTitle}>
          ...
          <TouchableOpacity
            style={[
              ...
              (!cardDetailsComplete || isProcessing) &&
